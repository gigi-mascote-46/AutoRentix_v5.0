            ...
            <section>
                ...
                <div class="p-4 border border-green-400 rounded bg-green-50">
                    ...
                    <div class="mb-4">
                        ...
                        <label class="inline-flex items-center mr-6">
                            <input type="radio" class="form-radio" value="cartao_credito"
                                v-model="form.metodo_pagamento" />
                            <span class="ml-2">Cartão de crédito</span>
                        </label>
                        <label class="inline-flex items-center">
                            <input type="radio" class="form-radio" value="referencia_multibanco"
                                v-model="form.metodo_pagamento" />
                    ...
                    <div v-if="form.metodo_pagamento === 'cartao_credito'">
                        ...
                        <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                            <div>
                                ...
                                <input id="cartao_numero" v-model="form.cartao_numero" type="text" required
                                    class="input" />
                            </div>
                            <div>
                                <label for="cartao_validade" class="block font-medium">Data de validade (MM/AA)*</label>
                                <input id="cartao_validade" v-model="form.cartao_validade" type="text" required
                                    class="input" placeholder="MM/AA" />
                            </div>
                            <div>
                    ...
                    <div v-if="form.metodo_pagamento === 'paypal'">
                        <p class="mb-2 font-semibold">Pagamento via PayPal será processado após a submissão do
                            formulário. (API PayPal precisa ser instalada)</p>
                    </div>
...
onMounted(() => {
    axios.get('/api/localizacoes')
        .then(response => {
            localizacoes.value = response.data;
        })
        .catch(error => {
            console.error('Failed to fetch localizacoes:', error);
        });
});
...
const submitPayment = () => {
    ...
    router.post('/areacliente/reservas', {
        ...
        onSuccess: () => {
            ...
            router.visit('/areacliente/reservas/finish-transaction', {
                data: {
                    amount: props.total.toFixed(2),
                    payerName: form.nome + ' ' + form.apelido,
                }
            });
        },
        onError: (errors) => {
            console.log('Error creating reservation', errors);
            alert('Erro ao efetuar a reserva: ' + JSON.stringify(errors));
        }
    });
};
</script>
