    ...
    <div class="flex flex-col min-h-screen">
        ...
        <header class="flex items-center justify-between p-4 bg-white shadow">
            ...
            <nav class="space-x-4">
                ...
                <Link href="/about" class="hover:text-blue-600">Sobre</Link>
                <Link href="/contact" class="hover:text-blue-600">Contacto</Link>
                ...
                <template v-else>
                    <Link href="/client/dashboard" class="font-semibold text-blue-600 hover:underline">
                    Dashboard <span v-if="auth && auth.user">({{ auth.user.name }})</span>
                    </Link>
        ...
        <button @click="toggleChat"
            ...
            aria-label="Open chat" title="Abrir chat de ajuda">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-7 w-7" fill="none" viewBox="0 0 24 24"
                stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round"
                    d="M8 10h.01M12 10h.01M16 10h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72A7.963 7.963 0 013 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
            </svg>
        </button>
        ...
        <div v-if="chatOpen"
            ...
            <div class="flex items-center justify-between px-4 py-2 text-white bg-blue-600 rounded-t-lg">
                ...
                <button @click="toggleChat" aria-label="Fechar chat" title="Fechar chat"
            ...
            <div class="flex-1 p-4 space-y-3 overflow-y-auto max-h-64" ref="messagesContainer">
                <div v-for="message in messages" :key="message.id"
                    :class="{ 'text-right': message.sender_id === userId }">
                    <div :class="[
            ...
            <form @submit.prevent="sendMessage" class="flex p-2 border-t border-gray-300">
                <input v-model="newMessage" type="text" placeholder="Escreva a sua mensagem..."
            ...
            <!-- Remove login warning to allow guest messages -->
...
const sendMessage = async () => {
    ...
    try {
        ...
        // Optionally display AI response in chat if returned
        if (response.data.ai_response) {
            messages.value.push({
                id: Date.now(),
                sender_id: adminUserId.value,
